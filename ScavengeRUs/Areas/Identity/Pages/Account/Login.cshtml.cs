...
namespace ScavengeRUs.Areas.Identity.Pages.Account
{
    public class LoginModel : PageModel
    {
        ...
        {
            returnUrl ??= Url.Content("/Hunt");
            string userReturnUrl = Url.Content("~/");
            ...
            if (ModelState.IsValid)
            {
                ...
                else
                {
                    if (result.Succeeded)
                    {
                        ...
                        return LocalRedirect(userReturnUrl);

                    }
                    if (result.RequiresTwoFactor)
                    {
                        return RedirectToPage("./LoginWith2fa", new { ReturnUrl = userReturnUrl, RememberMe = Input.RememberMe });
                    }
                    if (result.IsLockedOut)
                    {
                        _logger.LogWarning("User account locked out.");
